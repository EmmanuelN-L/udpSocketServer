...
def connectionLoop(sock):
    ...
    while True:
        ...
        data = str(data, 'utf-8') #Took me too long to figure this out 
        print("Got this: " + data)
        ...
        if addr in clients:
            ...
            if 'heartbeat' in data:
                ...
                #If localPos is in the data it go to this code
                #Now the positions are able to update constantly
            if 'localPos' in data: 
                print("Got this: " + data)
                #Converting the json string to a dictionary
                positionData = json.loads(data) 

                clients[addr]['playerPosition'] = {
                    "x": positionData['localPos']['x'],
                    "y": positionData['localPos']['y'],
                    "z": positionData['localPos']['z']
                }
        else:
...
def gameLoop(sock):
    ...
    while True:
        ...
        for c in clients:
            ...
            #Each cube will get a random color but because of the client code this will only happen once
            clients[c]['color'] = {
                "R": random.random(),
                "G": random.random(),
                "B": random.random()
        ...
        if (len(clients) > 0):
            pktID = pktID + 1
        #This is for the 30 update packets a second this made the colours go quick
        #time.sleep(1/3) Slow af
        #time.sleep(1/10) Aight
        time.sleep(1/30) #Rave time 
